...
export async function sendCourseRejectedEmail({
  ...
  console.log('📧 [sendCourseRejectedEmail] Enviando a:', to)

  const resend = getResend()
  if (!resend) {
    console.warn('⚠️ [sendCourseRejectedEmail] Email no enviado: Resend no configurado')
  ...
  try {
          ...
          <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
            ...
            <div style="background: linear-gradient(135deg, rgba(251,191,36,0.1) 0%, rgba(247,147,26,0.1) 100%); border: 1px solid rgba(251,191,36,0.3); border-radius: 16px; padding: 32px;">
              ...
              <p style="color: #d1d5db; font-size: 16px; line-height: 1.6; margin: 0 0 16px 0;">
                Hemos revisado tu curso y necesita algunos ajustes antes de poder publicarlo:
              </p>
              ...
              <!-- Motivo del rechazo -->
              <div style="background: rgba(0,0,0,0.3); border-left: 4px solid #fbbf24; padding: 16px; border-radius: 0 8px 8px 0; margin-bottom: 24px;">
                <p style="color: #9ca3af; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px; margin: 0 0 8px 0;">
    ...
    if (error) {
      console.error('❌ [sendCourseRejectedEmail] Error:', error)
      return { success: false, error: error.message }
    }

    console.log('✅ [sendCourseRejectedEmail] Enviado:', data?.id)
    return { success: true, id: data?.id }

  } catch (error) {
    console.error('❌ [sendCourseRejectedEmail] Error crítico:', error)
