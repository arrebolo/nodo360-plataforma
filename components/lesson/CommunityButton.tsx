  ...
  if (isCompact) {
    return (
      <div className="space-y-4">
        ...
        {/* Stats */}
        <div className="bg-dark-secondary/50 rounded-lg p-3 border border-dark-border">
          <div className="flex items-center gap-2 mb-1">
            <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse" />
            <span className="text-sm font-medium text-white">{memberCount} miembros activos</span>
          </div>
          <p className="text-xs text-white/60">En {platformName}</p>
        </div>
            ...
            {isRegistered && (
              <div className="flex flex-wrap gap-2">
                {features.slice(0, 4).map((feature, index) => (
                  <span
                    key={index}
                    className="inline-flex items-center gap-1 px-2 py-1 bg-dark-secondary/50 rounded-lg text-xs text-white/80"
                  >
