...
- [ ] **No hay forma de saber si un endpoint se consultó, y con el plan actual no la habrá.** El incidente del 24/09/2026 dejó una ventana de 10 meses sin ninguna manera de comprobar si alguien la aprovechó. Los tres sitios donde debería haber quedado rastro fallan por motivos distintos:
  - **Vercel, plan Hobby: no conserva histórico.** Los registros de ejecución son una ventana en vivo, no un archivo, y los *Log Drains* no están disponibles en Hobby. No es que estén mal configurados: no se pueden configurar. Mientras el proyecto siga en Hobby, **cualquier incidente futuro se documentará igualmente a ciegas**.
  - **Supabase**: los registros de API duran 1 día en el plan gratuito y 7 en Pro. Sirven para un incidente que se detecta el mismo día, para nada más.
  - **La aplicación**: no audita lecturas en ningún endpoint.

  Las opciones, por si algún día importa: subir a Vercel Pro y configurar un *Log Drain* al almacenamiento más barato; o registrar en la propia base los accesos a las rutas que sirven datos personales, que es más trabajo pero no depende del plan. Ninguna es urgente hoy con 23 usuarios; las dos lo serán el día que haya datos de pago. **Decidirlo a propósito, no por omisión.**
...
- [ ] **Auditar todas las consultas que traen `courses`, `modules` o `lessons` como relación embebida sin `!inner` y asumen que la fila existe.** En PostgREST ese embed es un LEFT JOIN: cuando RLS oculta la fila —y la oculta en cuanto el curso no está `published`— llega `null`, no un error. El código que hace `x.course.title` revienta con 500, y el que hace `{...x.course}` produce un objeto vacío que se pinta como una tarjeta sin enlace.
  ...
  *(24/09/2026: el criterio ya está decidido y escrito — **regla 24** del prompt maestro: `!inner` cuando la fila es imprescindible, `?.` con respaldo explícito cuando no. Lo que sigue pendiente es la **ronda de aplicación**: recorrer los embeds existentes y clasificarlos uno a uno, y convertirlo en una comprobación repetible en vez de una revisión manual, porque el patrón vuelve cada vez que se archiva o despublica algo.)*
...
- [x] ~~Decidir destino de la ruta **Trading**~~ *(25/09/2026: fusionada en un curso, «Trading: qué es y por qué casi nadie gana», con un límite escrito: responde a si operar, nunca a cómo. Fuera el armazón operativo y los tres juicios que chocaban con el Principio #1; dentro apalancamiento y obligaciones fiscales, que faltaban y son necesarios para decidir. Migración 052)*
- [x] ~~Reencuadrar «Seguridad básica en Bitcoin y criptomonedas»~~ *(24/09/2026: hecho en la 048, con la fusión de los dos cursos de custodia)*
- [ ] **El mapa de 8 rutas sigue sin suelo: falta *Fundamentos Blockchain*.** Los dos cursos de Web3 fusionados en la 052 dan por sabido qué es una blockchain y remiten a *Cómo funciona Bitcoin*, que habla de Bitcoin y no de blockchains en general. La lección 3.3 del curso nuevo reconoce el hueco por escrito en vez de prometer fechas. Mientras no exista, Tier 3 entero (Ethereum, DeFi, NFTs, DAOs, L1/L2) se apoya en el aire.
...
- [ ] **Los certificados emitidos conservan el título antiguo del curso, y el de verificación muestra el nuevo.** Tras la 052, `NODO-20260120-N97P5` dice «Introducción al trading de criptomonedas» en su columna `title` y `/verificar` muestra «Trading: qué es y por qué casi nadie gana», porque la función lee el título actual del curso. Ninguna de las dos opciones es obviamente correcta: mostrar el título de entonces es más fiel a lo que se completó, mostrar el actual es más útil para quien verifica. Hoy son 4 certificados en esa situación; decidir el criterio antes de que sean cuarenta.
