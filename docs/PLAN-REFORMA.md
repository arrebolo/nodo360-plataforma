...
- [ ] Decidir el tratamiento de los cursos `coming_soon`: aparecen en `/cursos` (los lista `getAllCourses` con `.in('status', ['published','coming_soon'])`) pero su ficha da 404, porque la regla de visibilidad de `lib/courses/access.ts` solo abre los `published`. Opciones: ficha propia sin lecciones, o no listarlos. Valorar contra el Principio #7 (no prometer). Hoy no hay ningún curso en ese estado, así que no rompe nada todavía
